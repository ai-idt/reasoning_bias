import os
os.environ["CUDA_VISIBLE_DEVICES"] = "0" #Do this to select the GPU(s) llama_cpp will use
...
llm = Llama(model_path="/mnt/Drive4/bene/Qwen3-32B-Q8-qwen.gguf", #https://huggingface.co/Qwen/Qwen3-32B-GGUF
...
for question in test_df:
    ...
    cur_prompt_think = cur_prompt + " /think" #Qwen3 uses this to explicitly turn on or off thinking mode
    cur_prompt_nothink = cur_prompt + " /no_think"

    case_done = False
    while not case_done:

        response = llm.create_chat_completion(messages = [{"role": "user", "content": cur_prompt_think}],
                    temperature=0.6, presence_penalty=1.5, top_p=0.95, top_k=20, min_p = 0., #Qwen recommendations
                    max_tokens=32768)

        #Check that answer is correct, i.e., last letter is in acceptable_answers
        if len(response["choices"][0]["message"]["content"]) > 0:            
            if response["choices"][0]["message"]["content"][-1] in acceptable_answers:
                case_done = True

    res_dict = {"Model": "Qwen3_Think",
    ...
    while not case_done:
        ...
        response = llm.create_chat_completion(messages = [{"role": "user", "content": cur_prompt_nothink}],
                    temperature=0.7, presence_penalty=1.5, top_p=0.8, top_k=20, min_p = 0., #Qwen recommendations
                    max_tokens=32768)
